...
use super::common::{CommonBuildArgs, generate_build_result_for_command, add_schema_to_plan_json, write_json_file};
...
pub fn run_plan(args: &PlanArgs) -> crate::Result<()> {
    ...
    let plan = result.plan.ok_or_else(|| {
        crate::ArcpackError::Other(anyhow::anyhow!("BuildResult missing plan"))
    })?;
